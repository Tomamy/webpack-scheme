var tpl = "<div id=\"upload-layer\" class=\"upload-layer simplemde-widget\"> <iframe name=\"upload-iframe\" style=\"display:none\"></iframe> <div id=\"upload-imgpanel\"> <form action=\"/upload\" id=\"upload-form\" method=\"post\" name=\"upload-form\" enctype=\"multipart/form-data\" target=\"upload-iframe\"> <div class=\"head-wrap\"> <div class=\"text\">插入图片</div> <div class=\"close\"><i class=\"glyphicon glyphicon-remove\"></i></div> </div> <div class=\"content-wrap\"> <ul class=\"nav nav-tabs\"> <li role=\"presentation\" class=\"active\" data-name=\"local\"><a href=\"javascript:;\">本地上传</a></li> <li role=\"presentation\" data-name=\"remote\"><a href=\"javascript:;\">远程地址获取</a></li> </ul> <div class=\"tabs-panel\"> <p>图片大小不大于2M</p> <div class=\"row\"> <div class=\"col-7\"> <div class=\"address form-control\" disabled>选择的图片</div> </div> <div class=\"col-3\"> <div class=\"btn btn-default\">选择图片</div> <input type=\"file\" name=\"imgs\" required/> </div> </div> </div> </div> <div class=\"footer-wrap\"> <div class=\"row\"> <div class=\"col-7 text-right\"> <div class=\"mesg text-info\"></div> </div> <div class=\"col-5 text-right\"> <div class=\"btns\"><div class=\"btn btn-default cancel\">取消</div> <div class=\"btn btn-primary insert\"> 插入</div></div> </div> </div> </div> </div> </form> </div> </div>",
...
function run(url,i_loading,callback){
	...
	upload_url = url;
	$tpl = $(tpl);
	$tpl.find('#upload-form').append('<input type="hidden" name="_token" value="'+$('body').data('_token')+'">');
	$("body").append($tpl);         
	//event agent 
...
function uploadHandler($target){
	...
	(function(){
		timer = setInterval(function(){
			...
			var docstr = $idoc.find("body").text();
			console.log(docstr);
			if(docstr){
				clearInterval(timer); 
				$target.html("插入");
